        ...
        public override string Status
        {
            get
            {
                ...
                foreach (ConcurrentQueue<TcpServerPayload> sendQueue in m_clientInfoLookup.Values.Select(clientInfo => clientInfo.SendQueue))
                    statusBuilder.AppendLine($"           Queued payloads: {sendQueue.Count:N0} for client {++count:N0}");
            ...
            try
            {
                ...
                if (IntegratedSecurity)
                {
                    ...
                    try
                    {
                        ...
                        cancelTimeout = new Action(() => client.Provider?.Dispose()).DelayAndExecute(15000);
